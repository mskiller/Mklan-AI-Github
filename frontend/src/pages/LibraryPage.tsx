...
const galleryTabGroups: GalleryTabGroup[] = ["Operations"];
...
export function LibraryPage() {
  ...
  const [activeTab, setActiveTab] = useState<GalleryTab>("sources");
        ...
        <div style={{ display: "flex", alignItems: isMobile ? "stretch" : "center", justifyContent: "space-between", gap: "1rem", flexDirection: isMobile ? "column" : "row" }}>
          <div>
            <h1 style={{ fontSize: "1.75rem", fontWeight: 700, margin: 0, letterSpacing: "-0.02em" }}>
              {language === "fr" ? "Gestion de la Bibliothèque" : "Media Library Management"}
            </h1>
            <p style={{ margin: "0.25rem 0 0", color: "var(--text-secondary)", fontSize: "0.9rem" }}>
              Configure indexed folders, queue background scans, and view scan worker jobs
            </p>
          </div>
        </div>

        <nav style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "minmax(220px, 1fr)", gap: "0.55rem", padding: "0.5rem", background: "rgba(255,255,255,0.025)", border: "1px solid var(--border-color)", borderRadius: "14px" }}>
        ...
        {activeTab === "sources" && (
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "minmax(180px, 240px) minmax(260px, 1fr) auto", gap: "0.75rem", padding: "1rem", background: "rgba(255,255,255,0.025)", border: "1px solid var(--border-color)", borderRadius: "12px", alignItems: "end" }}>
            ...
            <div style={{ gridColumn: "1 / -1", color: "var(--text-secondary)", fontSize: "0.86rem", lineHeight: 1.5 }}>
              Paste a Windows path for a local folder or a mapped network drive. The library only reads the originals; metadata, previews, and the search index are stored separately.
            </div>
            {sourceMessage && (
              <div
                style={{
              ...
              <>
                ...
                {sourceDirectories.length > 0 ? (
                  ...
                  <div style={{ color: "var(--text-muted)", fontSize: "0.86rem" }}>
                    No subfolders in this location. Use Live Browse folder explorer above to navigate.
                  </div>
                )}
              </>
            )}
