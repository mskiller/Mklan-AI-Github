"""Database connections and initialisation for Mklan Studio."""
...
def connect_wildcard() -> sqlite3.Connection:
    """Connect to the Wildcards SQLite database."""
    config.WILDCARD_DATA_DIR.mkdir(parents=True, exist_ok=True)
    conn = sqlite3.connect(config.WILDCARD_DB)
...
def connect_movie() -> sqlite3.Connection:
    """Connect to the Movie tool SQLite database."""
    conn = sqlite3.connect(config.MOVIE_DB)
    conn.row_factory = sqlite3.Row
    conn.execute("PRAGMA journal_mode=WAL")
    conn.execute("PRAGMA synchronous=NORMAL")
    conn.execute("PRAGMA foreign_keys=ON")
...
def init_wildcard_db() -> None:
    """Initialise the Wildcards database schema."""
    conn = connect_wildcard()
    try:
        conn.executescript(
            """
...
def init_db() -> None:
    """Initialise all database schemas. Safe to call repeatedly."""
    init_wildcard_db()